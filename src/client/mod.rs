...
pub async fn run_client(
    ...
    let http_client = Client::builder().add_root_certificate(cert).build()?;
    ...
    loop {
        ...
        println!("New transfer configuration: {:#?}", transfer_config);
            ...
            "upload" => {
                ...
                for file_id in 1..=transfer_config.num_files {
                    ...
                    let body_stream = stream::iter((0..chunks).map(move |chunk_id| {
                        ...
                        let data = file_generator::generate_file(chunk_size);

                        Ok::<Frame<Bytes>, Infallible>(Frame::data(Bytes::from(data)))
                    }));

                    let body: BoxBody<Bytes, BoxError> = StreamBody::new(body_stream)
                    ...
                    println!("HTTP connection ready for upload");
                    ...
                    println!("Uploaded file {}/{}", file_id, transfer_config.num_files);
                }
            }

            "download" => {
                for file_id in 1..=transfer_config.num_files {
                    let body: BoxBody<Bytes, BoxError> =
                    ...
                    println!("HTTP connection ready for download");
                    ...
                    println!(
                        ...
                        file_id, transfer_config.num_files, received_bytes
                    );
                }
            }

            _ => {
                eprintln!("Unknown operation: {}", transfer_config.operation);
            }
        }
