...
impl AlgorithmProvider for ClassicalProvider {
    ...
    fn build_server_config(&self, selected_cipher_suites: &[String], selected_kx_groups: &[String]) -> Result<ServerConfig, Box<dyn std::error::Error>> {
        ...
        let classic_server_config = ServerConfig::builder_with_provider(Arc::new(my_crypto_provider)).with_protocol_versions(&[&version::TLS13])?.with_no_client_auth().with_single_cert(cert_chain, server_private_key)?;

        Ok(classic_server_config)
